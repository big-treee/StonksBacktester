...
class ZerodhaCharges(BaseBrokerCharges):
    def calculate_charges(
        ...
        brokerage = 0.0
        stt = 0.0
        exc = 0.0000345 * trade_value  # nse exchange txn charge.
        sebi = 0.000001 * trade_value  # rs per crore.
        stamp = 0.0
        ...
        if product == "cnc":
            ...
            brokerage = min(0.0003 * trade_value, 20.0)
            if not is_buy:
                stt = 0.000125 * trade_value
            exc = 0.00002 * trade_value
            if is_buy:
                stamp = 0.00002 * trade_value
        elif product == "options":
            brokerage = 20.0
            if not is_buy:
                stt = 0.000625 * trade_value  # stt on premium.
            exc = 0.00053 * trade_value
            if is_buy:
                stamp = 0.00003 * trade_value
